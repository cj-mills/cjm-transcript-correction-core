...
async def load_document_segments(
    ...
    return [_row_to_spine_segment(r) for r in (field_of(result, "rows", []) or [])]
...
async def load_review_markers(
    ...
    return {row[0]: row[1] for row in (field_of(r, "rows", []) or [])}
